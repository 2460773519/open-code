...
/*********************************************************************
    CONSTANTS
*/
#define RF_RFPHY_TX_ONLY                   (0x00)
#define RF_RFPHY_RX_ONLY                   (0x01)
#define RF_RFPHY_TRX_ONLY                  (0x02)
#define RF_RFPHY_RX_TXACK                  (0x03)
#define RF_RFPHY_IDLE                      (0xFF)
...
#define RF_PKT_FMT_100K                    (0x06)

#define RF_HW_MAX_RX_TO                    (20000)
int read_time=37;
...
/*********************************************************************
    LOCAL VARIABLES
*/
uint8 PhyRf_TaskID; // Task ID for internal task/event processing
...
{
    if(s_phy.Status==RF_RFPHY_RX_TXACK)
    {
        //process data check
        return TRUE;
    }

    osal_set_event(PhyRf_TaskID,PPP_RX_DATA_PROCESS_EVT);
    return 0;
}
...
void phy_rf_tx(void)
{
    ...
    if(s_phy.Status==RF_RFPHY_TRX_ONLY)
    ...
    phy_hw_go();
    llWaitingIrq=TRUE;
    HAL_EXIT_CRITICAL_SECTION();//使能传输中断
}
void phy_rf_rx(void)
{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    ...
	    if(phyBufRx[3]==0x00&&phyBufRx[4]==0x00&&phyBufRx[5]==0x00)
			{
      //  LOG("-------------------------\n");
//				 hal_gpio_write(GPIO_P07,temp=!temp);	
        //LOG("[PHY RX] [-%03ddbm %4dKHz %02d CH] ",phyRssi,phyFoff-512,s_phy.rfChn);
    //    saomiao_sent();
        for(uint8_t i=0; i<29; i++)
				{
				   phyBufTx[i]=phyBufRx[i];
          // LOG(" %02x ",phyBufRx[i]);
				}
       // LOG("\n");
			}
...
void PLUSPHY_IRQHandler(void)
{
    ...
    if (mode == LL_HW_MODE_STX  &&
            (s_phy.Status == RF_RFPHY_TX_ONLY)
       )
    {
        osal_set_event(PhyRf_TaskID,PPP_TX_DONE_EVT);
    }
    else if(mode == LL_HW_MODE_SRX  &&
            (s_phy.Status == RF_RFPHY_RX_ONLY)
           )
    {
        ...
        osal_set_event(PhyRf_TaskID,PPP_RX_DONE_EVT);
    }
    else if(mode == LL_HW_MODE_TRX  &&
            (s_phy.Status == RF_RFPHY_TRX_ONLY)
           )
    {
        rf_phy_get_pktFoot(&phyRssi,&phyFoff,&phyCarrSens);
        ...
        osal_set_event(PhyRf_TaskID,PPP_TRX_DONE_EVT);
    }
//LOG("222");
    // post ISR process
    ll_hw_clr_irq();
...
void PhyRf_Init(uint8 task_id)
{
    PhyRf_TaskID = task_id;
 ...
 //  VOID osal_start_timerEx(PhyRf_TaskID, PPP_PERIODIC_TX_EVT, 1000);
  ...
  //  LOG("[PHY] init done %d rfchn%d SW[%8x] CRC[%d %8x] WT[%2x]\n"\
    ,s_phy.Status,s_phy.rfChn,s_pktCfg.syncWord,s_pktCfg.crcFmt, s_pktCfg.crcSeed,s_pktCfg.wtSeed);
	  ...
	  hal_gpio_write(GPIO_P07,1);	
	  hal_gpioretention_register(GPIO_P07);//
}
...
*/
uint8 ble_data=0;
uint16 PhyRf_ProcessEvent(uint8 task_id, uint16 events)
{
    VOID task_id;
	
   static uint32 time_cnt=0,ttt=0,temp=0;
    if (events & PPP_PERIODIC_TX_EVT)
    {
        if(s_phy.Status==RF_RFPHY_IDLE)
        {
					//LOG("111");
//            s_phy.Status = RF_RFPHY_TX_ONLY;
...
//					//ble_data++;
//				  // adv_buffer[0]=ble_data;  //修改要广播的数据
//					 //phy_tx_buf_updata(s_pubAddr,advHead,adv_buffer,31);
//            phy_rf_tx();
							 hal_gpio_write(GPIO_P07,temp=!temp);	
            osal_start_timerEx(PhyRf_TaskID,PPP_PERIODIC_TX_EVT,500);
        }
        else
        {
         //   LOG("SKIP TX_EVT Current Stats %d\n",s_phy.Status);
            osal_start_timerEx(PhyRf_TaskID,PPP_PERIODIC_TX_EVT,20);
        }
    ...
    if (events & PPP_PERIODIC_RX_EVT)
    {
        if(s_phy.Status==RF_RFPHY_IDLE)
        {
					 ...
					 ttt++;
					 if(ttt>=1000/read_time)
					 {
						 ttt=0;
						 hal_gpio_write(GPIO_P07,(time_cnt=!time_cnt));	
            ...
            s_pktCfg.wtSeed = WHITEN_SEED_CH37;
            phy_rf_rx();
            osal_start_timerEx(PhyRf_TaskID,PPP_PERIODIC_RX_EVT,read_time);
        }
        else
        {
        //    LOG("SKIP RX_EVT Current Stats %d\n",s_phy.Status);
            osal_start_timerEx(PhyRf_TaskID,PPP_PERIODIC_RX_EVT,20);
        }
